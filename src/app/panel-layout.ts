...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
    // Grid's Eye View (energy variant) is a focused, single-purpose app.
    // It intentionally hides the cross-variant switcher, attribution, GitHub
    // link, region picker, and share/copy button that the World Monitor shell
    // exposes. Branding is also swapped from "World Monitor" to
    // "Grid's Eye View" and the version is pinned to the Grid's Eye release.
    const isEnergy = SITE_VARIANT === 'energy';
    const brandName = isEnergy ? "Grid's Eye View" : 'World Monitor';
    const brandNameUpper = isEnergy ? "GRID'S EYE VIEW" : 'WORLD MONITOR';
    const brandLogoShort = isEnergy ? "GRID'S EYE VIEW" : 'MONITOR';
    const displayedVersion = isEnergy ? '1.0' : __APP_VERSION__;
    this.ctx.container.innerHTML = `
      ...
      <div class="header">
        <div class="header-left">
          ...
          ${isEnergy ? '' : `<div class="variant-switcher">${(() => {
        ...
        return `
          ...
          <span class="logo">${brandLogoShort}</span><span class="logo-mobile">${brandName}</span><span class="version">v${displayedVersion}</span>${BETA_MODE ? '<span class="beta-badge">BETA</span>' : ''}
          ${isEnergy ? '' : `<a href="https://x.com/eliehabib" target="_blank" rel="noopener" class="credit-link">
            <svg class="x-logo" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            <span class="credit-text">@eliehabib</span>
          </a>`}
          ${isEnergy ? '' : `<a href="https://github.com/koala73/worldmonitor" target="_blank" rel="noopener" class="github-link" title="${t('header.viewOnGitHub')}">
          ...
          ${isEnergy ? '' : `<div class="region-selector">
        ...
        <div class="header-right">
          ...
          ${this.ctx.isDesktopApp || isEnergy ? '' : `<button class="copy-link-btn" id="copyLinkBtn">${t('header.copyLink')}</button>`}
      ...
      <nav class="mobile-menu" id="mobileMenu">
        <div class="mobile-menu-header">
          <span class="mobile-menu-title">${brandNameUpper}</span>
          <button class="mobile-menu-close" id="mobileMenuClose" aria-label="Close menu">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        ${isEnergy ? '' : `<div class="mobile-menu-divider"></div>
        ${(() => {
        const variants = [
        ...
        ${isEnergy ? '' : `<a class="mobile-menu-item" href="https://x.com/eliehabib" target="_blank" rel="noopener">
        ...
        <div class="mobile-menu-version">v${displayedVersion}</div>
      </nav>
        ...
        <div class="map-section" id="mapSection">
          <div class="panel-header">
            <div class="panel-header-left">
              ${isEnergy ? '' : `<span class="panel-title">${SITE_VARIANT === 'tech' ? t('panels.techMap') : SITE_VARIANT === 'happy' ? 'Good News Map' : t('panels.map')}</span>`}
            </div>
      ...
      ${isEnergy ? `<footer class="site-footer">
        <div class="site-footer-brand">
          <img src="/favico/favicon-32x32.png" alt="" width="28" height="28" class="site-footer-icon" />
          <div class="site-footer-brand-text">
            <span class="site-footer-name">${brandNameUpper}</span>
            <span class="site-footer-sub">v${displayedVersion}</span>
          </div>
        </div>
  ...
  private createNewsPanel(key: string, labelKey: string): NewsPanel | null {
    ...
    // Prefer the variant-aware display name from panelSettings (e.g.
    // "Energy News" under Grid's Eye View) over the default i18n label.
    const variantName = this.ctx.panelSettings[key]?.name;
    const label = variantName ?? t(labelKey);
  ...
  private createPanels(): void {
    ...
    this.createPanel('energy-stats', () => new EnergyStatsPanel());
